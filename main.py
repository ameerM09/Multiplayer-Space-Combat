...
return_menu_btn = Button(5, 8.75, RETURN_MENU_BTN)

# Function to draw and update elements on screen
def draw_elements(win, player_blue, player_red, blue_projectile, red_projectile, player_blue_health, player_red_health, menu_bar):
    ...
    restart_btn = Button(50, 7.5, RESTART_BTN)
...
# Additional conditions are added to add borders around screena
...
def render_game_winner(winning_img):
    WIN.blit(winning_img, (WIN_WIDTH // 2 - winning_img.get_width() // 2, 
            WIN_HEIGHT // 2 - winning_img.get_height() // 2))
...
# Set to the maximum volume by default
            ...
            WIN.blit(SPACE_BACKGROUND, (0, 0))
            render_game_winner(PLAYER_TWO_WIN)
            game_over = True
        ...
        if player_red_health <= 0:
            ...
            WIN.blit(SPACE_BACKGROUND, (0, 0))
            render_game_winner(PLAYER_ONE_WIN)
            game_over = True
...
# Variable to keep track of key bindings        
        ...
        draw_elements(WIN, player_blue, player_red, blue_projectile, red_projectile, player_blue_health, player_red_health, menu_bar)
...
def main_menu(win):
    ...
    play_btn = Button(WIN_WIDTH // 2 - MAIN_MENU_PLAY_BTN.get_width() // 2, 200, MAIN_MENU_PLAY_BTN)

    exit_btn = Button(WIN_WIDTH // 2 - EXIT_BTN.get_width() // 2, 325, EXIT_BTN)

    start_menu_scroll_bg = Scroller(0, 0)

    while run:
        start_menu_scroll_bg.drag_movement()

        start_menu_scroll_bg.render(win)

        win.blit(MAIN_MENU_LOGO, (WIN_WIDTH // 2 - MAIN_MENU_LOGO.get_width() // 2, 100))
